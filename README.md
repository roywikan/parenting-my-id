...
3. **Automated `llms.txt`, `llms-full.txt` & `feed.xml` RSS Synchronization**:
...
4. **Header Badge Customization (`show_header_badge`)**: Opsi toggle di Portal Admin (`/admin`) untuk menampilkan atau menyembunyikan elemen `<span>` badge "Cloudflare D1 Edge Engine" di samping logo header.
5. **Autolinks Engine**: Otomatis mengubah kata kunci tertentu di seluruh artikel menjadi internal link aktif tanpa perlu mengedit artikel satu per satu.
6. **Histori Revisi & Autosave**: Menyimpan draf artikel dan 3 histori revisi terakhir untuk perlindungan data tulisan penulis.
