...
Dokumen ini berisi panduan teknis lengkap mengenai cara melakukan instalasi, konfigurasi database Cloudflare D1, integrasi GitHub REST API, setting DNS, setting Worker/Pages Cloudflare, serta panduan keamanan sistem.

---

## 🛠️ Ringkasan Fitur Utama & Arsitektur
- **Frontend SPA**: React 18, Vite 6, Tailwind CSS v4, Lucide Icons, TypeScript.
- **Backend Edge**: Cloudflare Pages / Workers Function (`functions/api/[[path]].ts`).
- **Database**: Cloudflare D1 Serverless SQLite (`parenting-db`).
- **Penyimpanan Media**: Cloudinary API dengan fallback otomatis ke GitHub Storage (`public/uploads/`).
- **Sinkronisasi File Statis**: Otomatis meng-update `llms.txt`, `llms-full.txt`, `sitemap.xml`, dan `feed.xml` ke repositori GitHub via GitHub REST API.
- **Fitur Keamanan Utama**:
  1. **Anti Brute Force**: Rate limiting otentikasi login admin.
  2. **Anti XSS**: Sanitasi otomatis pada semua input Markdown & HTML.
  3. **Anti Leech & Hotlinking**: Validasi Referer & Header keaslian media.
  4. **Proteksi Custom Admin Suffix (`admin_url_suffix`)**: Path publik `/admin` otomatis di-redirect 301 ke `/admin-[suffix]` (default: `/admin-9999`).
...
## 🔒 6. Kebijakan Keamanan & Cara Setting Suffix Rahasia Admin (`admin_url_suffix`)
